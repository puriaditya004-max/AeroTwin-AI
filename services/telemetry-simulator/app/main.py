...
async def attach_redis_publisher(target: ScenarioRunner) -> None:
    if redis_async is None:
        return
    try:
        client = redis_async.from_url(settings.redisUrl, socket_connect_timeout=2, socket_timeout=2)
        await client.ping()
        ...
        raise RuntimeError("Redis is required for the simulator API") from exc
