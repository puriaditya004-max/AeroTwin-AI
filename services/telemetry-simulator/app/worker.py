...
class M1Worker:
    ...
    async def run(self) -> None:
        ...
        if default_scenario:
            await self.runner.start(default_scenario, mission_id=os.getenv("M1_DEFAULT_MISSION_ID", "MSN-LIVE-001"), correlation_id=__import__("uuid").uuid4().hex)
        ...
        try:
            while True:
                ...
                __import__("pathlib").Path("/tmp/m1-heartbeat").write_text(str(__import__("time").time()))
