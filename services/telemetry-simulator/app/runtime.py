...
class ScenarioRunner:
    ...
    async def start(self, name: str, seed: int | None = None, mission_id: str | None = None, correlation_id: str | None = None) -> dict[str, Any]:
        ...
        resolved_corr = correlation_id or uuid.uuid4().hex
        self._stop = asyncio.Event()
        self.state = RunState(
            scenario=name,
            status="running",
    ...
    async def _emit_tick(self, model: EngineModel) -> None:
        ...
        for frame in frames:
            offset = frame.timestamp - model.replay_epoch
            frame.timestamp = self.state.started_at + offset
            frame.ingestTimestamp = now
            frame.correlationId = f"{self.state.correlation_id}:{self.state.tick:06d}"
        self.state.tick += 1
        if skip_publish:
    ...
    async def follow_desired(self) -> None:
        ...
        if desired.get("running"):
            ...
            if not name:
                return
            if self.state.status == "running" and self.state.scenario == name and self.state.correlation_id == desired.get("correlationId"):
                return
            if self.state.status == "running":
